...
func checkClawMachine(Xa, Ya, Xb, Yb, Px, Py float64) int {
	...
	fmt.Println("A,B", int64(A), int64(B))
	fmt.Printf("A,B %.3f %.3f\n", A, B)

	if big.NewFloat(A).Cmp(big.NewFloat(float64(int64(A)))) == 0 && big.NewFloat(B).Cmp(big.NewFloat(float64(int64(B)))) == 0 {

		fmt.Println("Equal A,B", A, B)
...
func readInput(file string) {
	...
	for s.Scan() {
		...
		if strings.Contains(line, "Prize") {
			...
			fmt.Println("P", res)
			Px = Atoi(res[1]) + 10000000000000
			Py = Atoi(res[2]) + 10000000000000
