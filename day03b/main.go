...
func scanMuls(data []byte, atEOF bool) (advance int, token []byte, err error) {
	...
	for {
		...
		if data[i+0] == 'd' && data[i+1] == 'o' && data[i+2] == '(' && data[i+3] == ')' {
			advance = i + 4
			token = []byte("do()")
			fmt.Println(">> Found do() >>>>>>>>>", i, advance, string(data[advance]), string(token[:]))
			return
		}

		if data[i+0] == 'd' && data[i+1] == 'o' && data[i+2] == 'n' && data[i+3] == '\'' && data[i+4] == 't' && data[i+5] == '(' && data[i+6] == ')' {
			advance = i + 6
			token = []byte("don't()")
			fmt.Println(">> Found don't() >>>>>>>>>", i, advance, string(data[advance]), string(token[:]))
			return
		}
...
func readInput(file string) int {
	...
	for s.Scan() {
		...
		if ch == "do()" {
			enabled = true
			fmt.Println("do() -->", enabled)
			continue
		}
		if ch == "don't()" {
			enabled = false
			fmt.Println("don't() -->", enabled)
			continue
		}
		if enabled {
